...
class User(Base):
    ...
    # MySQL exige tamanho para VARCHAR. 255 é o padrão seguro.
    name = Column(String(255), index=True)
    email = Column(String(255), unique=True, index=True)
    hashed_password = Column(String(255))

    # --- NOVOS CAMPOS PARA CONTROLE DE ACESSO ---
    # Define o tipo de usuário: 'super_admin', 'reseller', 'user'
    role = Column(String(50), default="user")

    # Armazena as permissões de visualização do menu (controlado pelo admin)
...
class MonitoredURL(Base):
    ...
    error = Column(String(500), nullable=True) # Erros podem ser longos
